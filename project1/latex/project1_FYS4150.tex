...
\graphicspath{{../output/plots/old/}} % path for figures
...
\begin{figure}[h!]\label{fig:p1_analytical_sol}
    \centering
...
We implement the general algorithm, \Algo{p6_general_algorithm}, in C++ and let $A$ be the matrix from Problem 4, i.e. $a_i=c_i=-1$ and $b_i=2$. For a given number of discretization steps, $n_\text{steps}$, the code saves the solution as pairs $(x_i, v_i)$ to a file.
...
\begin{figure}[h!]
    ...
    \caption{The logarithm of the maximum relative error plotted over the logarithm of number of steps used in \Algo{p6_general_algorithm}.}\label{fig:p8_max_rel_err}
\end{figure}
