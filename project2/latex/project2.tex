...
Using our our symmetric, tridiagonal matrix $A \in \mathbb{R}^{N\cross N}$, for $N=2,3,\dots,100$ we run the Jacobi rotation algorithm (\hl{*} ?? change perhaps) and save the number of transformations (i.e. iterations in \hl{CHECK THIS} \hyperref[algo:jacobi_rotation]{Jacobi rotation}) needed for the matrix to be close enough\footnote{Choosing the number $\varepsilon=10^{-8}$ to be \textit{close enough} to zero.} to a diagonal matrix.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{jacobi_comparison.pdf}
    \caption{The number of transformations (or iterations) needed for the Jacobi rotation algorithm (ref?) to converge as function of matrix size $N$. NEED UPDATE IN PLOT.}\label{fig:p6_transformations_per_N}
\end{figure}
