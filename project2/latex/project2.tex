...
To test we write a short program in \CC\, that first sets up the tridiagonal $A$ for $N=6$, then solves $A\vec{v} = \lambda \vec{v}$ using the Armadillo library, where $\vec{v}$ and $\lambda$ are the eigenvectors and eigenvalues of $A$, respectively. Finally, we compare the eigenvectors and eigenvalues from Armadillo with the analytical result for $N=6$. The analytical solution we compare with is 
\begin{align*}
