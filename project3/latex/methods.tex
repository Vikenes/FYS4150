...
\begin{equation}\label{eq:trap_ext_B_field}
    ...
    \end{cases}.
\end{equation}
For the analytical derivations that follow in this section (section \ref{sec:penning_trap}), we will assume that $\abs{r}\leq d$ is fulfilled.  
...
\Vetle{Rewrite the below list. Should not be bullet points. Should perhaps not introduce features that are not yet relevant for the report, e.g. time-dep potential is not mentioned yet.}
