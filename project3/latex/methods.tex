...
The \texttt{PenningTrap}-class imitates the physical system that is the Penning trap, and accepts only input particles of type \texttt{Particle}. When filled with $N_\mathrm{p}\geq 1$, an object of \texttt{PenningTrap} is ready to simulate the evolution of the \texttt{Particle} object(s) for a given period of time and time step size with either a forward Euler or a fourth order Runge-Kutta numerical scheme, by computing the external electric and magnetic fields according to equations \eqref{eq:trap_ext_E_field} and \eqref{eq:trap_ext_B_field}. When interactions are included, it includes the additional electric field contribution according to equation \eqref{eq:interaction_field}.
