...
\Vetle{Move following to time-dep subsection:} For simulations with more than two particles, we initialise the Calcium ions randomly in the sense that their initial positions and velocities are normally distributed around the centre of the trap.


\subsection{Single particle simulation}\label{sec:simulation}
We begin by simulating a single particle, using the initial conditions for Particle 1 in section \ref{sec:initial_conditions}. We compare the resulting axial motion obtained from FE and RK4 with the analytical solution in equation \eqref{eq:p1_diffeq_solution_z}. \Vetle{Move analytical solution description from results to here.}
...
State and decsribe each simulation, much is already written in the results section
